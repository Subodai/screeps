...
module.exports.run = function(creep) {
    ...
    // if the creep doesn't have a flag name it needs one
    if (!creep.memory.flagName) {
        ...
        if (flags.length > 0) {
            for (var i in flags) {
                ...
                var _room = Game.rooms[flag.room.name];
                if (_room.memory.minersNeeded && _room.memory.minersNeeded > 0) {
                    // Get the list of creeps assigned to that room
                    var list = _.filter(Game.creeps, (creep) => creep.memory.role == this.role && creep.memory.flagName == flag.name);
                }
            }
        }
    }

    // If we're not delivering, go to our flag room and fetch
